  ...
  return (
    <section className="px-5 space-y-4">
      <div className="card p-5 space-y-4">
        ...
        {/* Prize milestone list */}
        <div className="grid grid-cols-1 gap-2">
          {sorted.map((prize, idx) => {
            const unlocked = idx < milestonesReached;
            const milestone = (idx + 1) * 10;
            const isNext = idx === milestonesReached;
              ...
              >
                ...
                <div className="flex-1 min-w-0">
                  ...
                  <p className="text-[11px] text-gray-400">
                    {unlocked
                      ? "Liberado!"
                      : isNext
                        ? `Proximo - falta ${(milestone - pct).toFixed(1)}%`
                        : `Libera em ${milestone}%`}
                  </p>
                </div>
                ...
                {unlocked && (
                  <motion.span
                    initial={{ scale: 0 }}
                    animate={{ scale: 1 }}
                    transition={{ type: "spring", stiffness: 300, delay: 0.2 * idx }}
