...
*Opcional se source for igual a api_externa.
source String Opcional Origem da transação. Envie api_externa para ignorar a validação de productHash (útil para integrações onde os produtos não são cadastrados na plataforma).
...
Recurso: Vendedor
Endpoint para consultar os dados públicos do cadastro do vendedor associado à API Key.

Obter Dados do Vendedor
GET

/api/v1/seller.php
Retorna informações do perfil do vendedor, como nome, documento e e-mail de contato.

Exemplo de Resposta (Sucesso)

{
    "name": "Nome da Loja ou Responsável",
    "company_name": "Razão Social Ltda (se houver)",
...
processing: O pagamento está sendo processado pela adquirente.
under_review: A transação está em análise de segurança manual.
failed: O pagamento foi recusado, cancelado ou expirou.
refunded: O valor da transação foi devolvido ao cliente.
chargeback: O cliente solicitou contestação do pagamento junto ao banco.
